...
import { RuntimeLoginPanel } from '../components/RuntimeLoginPanel';
...
export function ProjectsPage() {
  ...
  async function removeProject(project: Project) {
    if (!window.confirm(`سامانه «${project.name}» حذف شود؟ اگر اجرای ثبت‌شده داشته باشد فقط بایگانی می‌شود.`)) return;
    setActionLoading(true);
    try {
      const result = await api<{ deleted?: boolean; archived?: boolean; runCount?: number }>(`/api/projects/${project.id}`, { method: 'DELETE' });
      await loadProjects();
      if (result.archived) notify(`سامانه بایگانی شد چون ${(result.runCount || 0).toLocaleString('fa-IR')} اجرا به آن متصل است.`, 'info');
      else notify('سامانه حذف شد.', 'success');
    } catch (error) { notify(error instanceof Error ? error.message : 'حذف سامانه ناموفق بود.', 'error'); }
    finally { setActionLoading(false); }
  }
    ...
    <main className="mx-auto max-w-[1800px] space-y-5 p-4 sm:p-6">
      {loading ? <Loading /> : <div className="grid items-start gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <div className="space-y-5">{selected ? <>
          <Card><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="text-lg font-bold text-gray-900">{selected.name}</h2><Badge tone={selected.isActive ? 'green' : 'gray'}>{selected.isActive ? 'فعال' : 'بایگانی'}</Badge></div><p className="mt-1 font-mono text-xs text-gray-400" dir="ltr">{selected.code}</p></div><div className="flex gap-2"><Button variant="secondary" icon={<Edit3 className="h-4 w-4" />} onClick={() => editProject(selected)}>ویرایش</Button><Button variant="secondary" loading={actionLoading} icon={<Trash2 className="h-4 w-4" />} className="border-red-200 text-red-700 hover:bg-red-50" onClick={() => void removeProject(selected)}>حذف سامانه</Button></div></div><p className="mt-4 text-sm leading-7 text-gray-600">{selected.description || 'توضیحی برای این پروژه ثبت نشده است.'}</p></Card>
