...
export function ExtensionPage() {
  ...
  const configurationMissing = import.meta.env.PROD && !extensionId;
  ...
  return <div className="min-h-screen bg-gray-50">
    ...
    <main className="mx-auto max-w-3xl space-y-5 p-4 sm:p-6">
      {configurationMissing && user?.role === 'ADMIN' && <Card><div className="flex gap-3 text-amber-700"><TriangleAlert className="mt-1 h-5 w-5 shrink-0" /><div><h2 className="font-semibold">شناسه افزونه تنظیم نشده است</h2><p className="mt-1 text-sm leading-7">پس از Load unpacked، شناسه نمایش‌داده‌شده در chrome://extensions را در VITE_CHROME_EXTENSION_ID قرار دهید و وب‌اپ را دوباره build کنید.</p></div></div></Card>}
      ...
      <Card>
        <div className="flex flex-col items-center py-6 text-center">
          ...
          {detecting ? <div className="mt-7"><Loading text="در حال شناسایی افزونه…" /></div> : !status.installed ? <div className="mt-7 space-y-3">
            {webStoreUrl ? <a href={webStoreUrl} target="_blank" rel="noreferrer"><Button icon={<ExternalLink className="h-4 w-4" />}>نصب افزونه Chrome</Button></a> : <div className="mx-auto max-w-xl rounded-xl border border-amber-200 bg-amber-50 p-4 text-right text-sm leading-7 text-amber-900"><Badge tone="amber">نصب دستی موقت</Badge><ol className="mt-3 list-decimal space-y-1 pr-5"><li><span dir="ltr">chrome://extensions</span> را باز و Developer mode را روشن کنید.</li><li>Load unpacked را بزنید و پوشه <code dir="ltr" className="rounded bg-white px-1.5 py-0.5">apps/extension/dist</code> را انتخاب کنید.</li><li>شناسه افزونه را در <code dir="ltr" className="rounded bg-white px-1.5 py-0.5">VITE_CHROME_EXTENSION_ID</code> تنظیم و وب‌اپ را دوباره build کنید.</li></ol></div>}
          ...
          </div> : !status.compatible ? <div className="mt-7 space-y-3"><Badge tone="amber">نسخه افزونه ناسازگار است</Badge>{webStoreUrl ? <a href={webStoreUrl} target="_blank" rel="noreferrer"><Button icon={<ExternalLink className="h-4 w-4" />}>به‌روزرسانی از Chrome Web Store</Button></a> : <p className="text-sm leading-7 text-gray-600">نسخه جدید را build کنید و برای افزونه در chrome://extensions گزینه Reload را بزنید.</p>}</div>
