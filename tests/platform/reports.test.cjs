...
async function insertRun(pool, values) {
  const { insertRun: storeInsert } = require('../../shared/db/run-store.cjs');
  const created = await storeInsert(pool, {
    core: {
      project_id: values.projectId,
      environment_id: values.environmentId,
      test_file_path: values.path,
      requested_by: values.userId,
      status: values.status,
      source_approach: values.approach || 'IS',
      tool_kind: values.tool || 'PLAYWRIGHT',
      pack_id: values.pack || null,
      flow_id: values.flow || null,
    },
    request: { timeout_seconds: 120 },
    source: { source_snapshot: 'fixture' },
  });
  await pool.query(
    `UPDATE runs SET total_tests=$2, passed_tests=$3, failed_tests=$4, skipped_tests=0, duration_ms=$5,
            started_at=$6::timestamptz, completed_at=$6::timestamptz, requested_at=$6::timestamptz
      WHERE id=$1`,
    [
      created.id,
