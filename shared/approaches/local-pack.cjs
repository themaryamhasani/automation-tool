...
function packsRoot() {
  return path.resolve(repoRoot(), process.env.LOCAL_PACK_ROOT || 'runtime/packs');
}

function appsRoot() {
  return path.resolve(repoRoot(), process.env.LOCAL_APP_ROOT || 'runtime/apps');
}
...
`;
  const k6Load = `import http from 'k6/http';
...
`;
  const k6Health = `import http from 'k6/http';
import { check } from 'k6';
...
`;
  const k6Security = `import http from 'k6/http';
import { check } from 'k6';
...
export default function () {
  const base = (__ENV.AUTOMATION_RUNTIME_ORIGIN || __ENV.BASE_URL || 'http://127.0.0.1:4520').replace(/\\/$/, '');
...
`;
  ...
  const surfacePath = path.join(repoRoot(), 'apps/api/src/approaches/templates/cde-runtime-surface.mjs');
  ...
  writeIfMissing('scripts/k6/load.js', k6Load);
  writeIfMissing('scripts/k6/health.js', k6Health);
  writeIfMissing('scripts/k6/security-perf.js', k6Security);
  writeIfMissing('scripts/k6.js', k6Load);
...
function ensurePack(approach, key, meta = {}) {
  ...
  const packFile = path.join(root, 'pack.json');
  let existing = {};
  if (fs.existsSync(packFile)) {
    try { existing = JSON.parse(fs.readFileSync(packFile, 'utf8')); } catch { existing = {}; }
  }
  const info = {
    ...existing,
    approach: String(existing.approach || approach || 'CDE').toUpperCase(),
    key: existing.key || key,
    title: existing.title || meta.title || key,
    updatedAt: new Date().toISOString(),
  };
  fs.writeFileSync(packFile, `${JSON.stringify(info, null, 2)}\n`, 'utf8');
  return {
    ...info,
    root,
...
function describePack(approach, key) {
  ...
  const k6Rel = defaultK6Path(approach, key);
  return {
    id: key,
    title: pack.title,
    packKey: key,
    ...
    tools: {
      ...
      K6: {
        cwd: path.posix.dirname(k6Rel),
        script: path.posix.basename(k6Rel),
      },
...
function defaultK6Path(approach, key) {
  const root = packRoot(approach, key);
  const candidates = [
    'scripts/k6/load.js',
    'scripts/k6.js',
  ...
  for (const rel of candidates) {
    if (fs.existsSync(path.join(root, ...rel.split('/')))) return rel;
  }
  return 'scripts/k6/load.js';
}
